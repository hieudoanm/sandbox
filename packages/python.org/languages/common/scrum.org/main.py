...
def init_ui():
    ...
    with st.sidebar:
        ...
        with st.form(key="Form", clear_on_submit=True):
            uploaded_file = st.file_uploader("Upload", type=["md"], key="md_upload")
...
def init_vector_store():
    ...
    loader = UnstructuredMarkdownLoader(first_file)
    document = loader.load()
    docs = chromautils.filter_complex_metadata(document)
    ...
    vector_store = Chroma.from_documents(
        ...
        collection_name="md_v_db",  # Important if you want to reference the DB later
    )
...
def get_context_aware_prompt(context_chain: RetrieverOutputLike) -> Runnable:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "You are a helpful assistant that can answer the users questions. Use provided context to answer the question as accurately as possible:\n\n{context}",
            ),
            MessagesPlaceholder(variable_name="chat_history"),
            ("user", "{input}"),
        ]
